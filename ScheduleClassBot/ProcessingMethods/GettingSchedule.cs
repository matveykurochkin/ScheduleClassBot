...
internal class GettingSchedule
{
    ...
    private string GetTodaySchedule(string[] dayArr, DayOfWeek today)
    {
        ...
        {
            _addedToResponseText += "❗ВЫХОДНЫЕ, показано расписание на понедельник!❗\n\n";
            return dayArr[0];
        }
    ...
    private string GetTomorrowSchedule(string[] dayArr, DayOfWeek today)
    {
        ...
        {
            _addedToResponseText += "❗ЗАВТРА ВЫХОДНОЙ, показано расписание на понедельник!❗\n\n";
            return dayArr[0];
        }
    ...
    internal async Task GetScheduleForGroupPMI(ITelegramBotClient botClient, Message message, string textMessage)
    {
        try
        {
            ...
            if (textMessage == "Расписание на сегодня ПМИ-120"
                || textMessage == "/todaypmi")
                textMessage = GetTodaySchedule(DayOfWeekPmi, _today);

            if (textMessage == "Расписание на завтра ПМИ-120"
                || textMessage == "/tomorrowpmi")
