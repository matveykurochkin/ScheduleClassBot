...
internal class GettingSchedule
{
    ...
    private string GetTodaySchedule(string[] dayArr, DayOfWeek today)
    {
        ...
        {
            _addedText += "❗ВЫХОДНЫЕ, показано расписание на понедельник!❗\n\n";
            return dayArr[0];
        }
    ...
    private string GetTomorrowSchedule(string[] dayArr, DayOfWeek today)
    {
        ...
        {
            _addedText += "❗ЗАВТРА ВЫХОДНОЙ, показано расписание на понедельник!❗\n\n";
            return dayArr[0];
        }
    ...
    internal async Task GetScheduleForGroupPMI(ITelegramBotClient botClient, Message message, string textMessage)
    {
        try
        {
            ...
            _addedText = _numeratorAndDenominator == 0
            ...
            if (DayOfWeekPmi.Contains(textMessage))
            {
                switch (textMessage)
                {
                    case "Понедельник ПМИ-120":
                        await botClient.SendTextMessageAsync(message.Chat,
                            $"{_addedText}📌Расписание на Понедельник (Числитель)\n" +
                    ...
                    case "Вторник ПМИ-120":
                        await botClient.SendTextMessageAsync(message.Chat,
                            $"{_addedText}📌Расписание на Вторник (Числитель)\n" +
                    ...
                    case "Среда ПМИ-120":
                        await botClient.SendTextMessageAsync(message.Chat,
                            $"{_addedText}📌Расписание на Среду (Числитель)\n" +
                    ...
                    case "Четверг ПМИ-120":
                        await botClient.SendTextMessageAsync(message.Chat,
                            $"{_addedText}📌Расписание на Четверг (Числитель)\n" +
                    ...
                    case "Пятница ПМИ-120":
                        await botClient.SendTextMessageAsync(message.Chat,
                            $"{_addedText}📌Расписание на Пятницу (Числитель)\n" +
                            $"1⃣ пара 08:30 - 10:00\nЭлективные дисциплины по физической культуре и спорту пр\nЖив О.Г.\n" +
